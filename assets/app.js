...
function renderShell() {
    /* السايدبار — الأقسام مجمّعة لتقصير القائمة */
    const link = (c) => {
    ...
    const groups = CATEGORY_GROUPS.map(g => {
        const items = g.ids.map(id => CATEGORY_BY_ID[id]).filter(Boolean);
        if (!items.length) return '';
        return `<p class="nav-group-label">${esc(t[g.id] || '')}</p>${items.map(link).join('')}`;
    }).join('');

    $('sidebarNav').innerHTML = `
        <a class="nav-link" href="#/" data-nav="home">${ICON.home}<span>${esc(t.navHome)}</span></a>
        <a class="nav-link" href="#/category/troubleshooting" data-nav="category/troubleshooting">${ICON.faq}<span>${esc(cTitle(CATEGORY_BY_ID.troubleshooting))}</span><span class="nav-count">${ARTICLES.filter(a => a.category === 'troubleshooting').length}</span></a>
...
function viewHome() {
    ...
    const recent = recentArticles(3);
    const popular = popularArticles(3);
    const essential = ['accountCreation.mainAccount', 'fees.serviceFees', 'cashIn.cliq']
    ...
    /* الأقسام معروضة ضمن مجموعات — أهدأ للعين من شبكة واحدة كبيرة */
    const groupBlocks = CATEGORY_GROUPS.map(g => {
        const items = g.ids.map(id => CATEGORY_BY_ID[id]).filter(Boolean);
        if (!items.length) return '';
        return `
            <section class="section">
                <div class="section-head"><h2>${esc(t[g.id] || '')}</h2></div>
                    ...
                    ${items.map(c => {
                        ...
                        return `<a class="card" href="#/category/${c.id}">
                            ...
                            <span class="card-body">
                                ...
                                <span class="card-meta">${n} ${esc(t.topics)}</span>
                            </span>
                        </a>`;
                    }).join('')}
                </div>
    ...
    $('view').innerHTML = `
        ...
        <div class="wrap-wide">
            ${recent.length ? `
            <section class="section">
                <div class="section-head"><h2>${esc(t.recentlyViewed)}</h2></div>
